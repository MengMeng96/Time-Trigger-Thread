...
void rt_thread_exit(void)
{
    ...
    {
        /* 将时间片设置为0，标志当前TT线程已经结束本周期的运行，避免影响其他线程运行 */
        thread->remaining_tick = 0;
        /* 移除用于冲突检测的链表节点
         * BE线程里面也有这个节点，但是没有使用，移除或者不移除均可 */
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if(thread->rt_is_TT_Thread)
    {
        /* 如果优先级大于RT_THREAD_PRIORITY_MAX，即32，那么这一定是一个TT线程 */
        RT_ASSERT(thread->current_priority == RT_THREAD_PRIORITY_MAX);
        /* 这里只是设置了thread的number_mask字段，也就是二进制表示的线程优先级，但是TT线程不需要这个字段，rt_is_TT_Thread已经暗示了最高优先级 */
        set_running_TT_Thread_count(get_running_TT_Thread_count() + 1);
    }
    else
    {
...
rt_err_t rt_thread_yield(void)
{
    ...
    /* 由普通线程转为TT线程，写这个条件是为了避免极端情况下普通线程独占最高优先级，不进行线程替换 */
    ...
    /* 如果是普通线程的话，而且当前这一优先级的线程只有它一个，那么是不会让出CPU的，因为其他线程优先级都比它低 */
...
void rt_thread_timeout(void *parameter)
{
    ...
    /* thread check */
    RT_ASSERT(thread != RT_NULL);
    //TT线程的yield函数会使得BE线程的状态产生错误，这个断言不再适用
    RT_ASSERT(rt_object_get_type((rt_object_t)thread) == RT_Object_Class_Thread);
...
/* 求最大公约数
 * 放在这里是为了方便
 * 因为只在rt_TT_thread_time_collision_check函数中用到了 */
...
/*
 * 这是用来检测TT线程执行时间是否冲突的函数
 * 算法逻辑来自于王宁晨学长的硕士毕业论文第2章2.2.3.5小节
...
rt_bool_t rt_TT_thread_time_collision_check(rt_uint32_t exec_cycle, rt_uint32_t exec_offset, rt_uint32_t maxi_exec_time)
{
    ...
    /* 当添加一个新的TT线程的时候，检查所有已有的TT线程与其是否冲突 */

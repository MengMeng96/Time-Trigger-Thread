...
struct rt_thread
{
    ...
    rt_uint32_t user_data;                             /**< private user data beyond this thread */
				...
				/* 实时调度相关的信息 */
		/* 是否是TT线程 */
		rt_bool_t rt_is_TT_Thread;
		/* TT线程执行周期，每个周期内执行一次 */
		rt_uint32_t thread_exec_cycle;
		/* TT线程执行周期偏移，在每个周期的offset时刻开始执行 */
		rt_uint32_t thread_exec_offset;
		/* 当前线程宣称的最长执行时间，如果超出这个时间，则终止该线程，并且记录下来 */
		rt_uint32_t thread_maxi_exec_time;
		/* 记录实时线程下一次开始执行的时间 */
		rt_uint32_t thread_start_time;
		/* 用于跳表的链表节点数组 */
#if RT_TT_THREAD_SKIP_LIST_LEVEL > 1
		rt_list_t row[RT_TT_THREAD_SKIP_LIST_LEVEL - 1];
#endif
		rt_list_t* thread_skip_list_nodes[RT_TT_THREAD_SKIP_LIST_LEVEL];
		/* 用来做时间冲突判断 */
		rt_list_t time_collision_list;
};
